...
int main() {
	score s[num];
	cout << "Enter student name, ID, age, sex(M for man, F for woman),and scores for 3 students" 
		<< endl;
	for (int i = 0; i < num; i++) {
		cout << "student " << i + 1 << ": ";
		cin >> s[i].name >> s[i].ID >> s[i] .age>> s[i].sex >> s[i].korean >> s[i].english >> s[i].math;
